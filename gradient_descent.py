...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        while t < self.max_iter_ and delta >= self.tol_:
            ...
            avg_weights += f.weights

            delta = np.linalg.norm(f.weights - prev_weights)
            prev_weights = f.weights.copy()

            if val < best_val:
                best_val, best_weights = val, f.weights.copy()

            self.callback_(solver=self, weights_=f.weights.copy(), val=val, grad=grad, t=t, eta=eta, delta=delta)
            t += 1

        final_loss = f.compute_output(X=X, y=y)
        print(f"Final Loss for {f.__class__.__name__}: {final_loss}")
