...
class LogisticModule(BaseModule):
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        logits = np.exp(X @ self.weights_)
        grad = -np.sum((y - (logits / (1 + logits)))[:, None] * X, axis=0) / X.shape[0]
        return grad
...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        return fidelity_output + regularization_output
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        regularization_grad = self.lam_ * self.regularization_module_.compute_jacobian(**kwargs)
        return fidelity_grad + regularization_grad
    ...
    def weights(self, weights: np.ndarray) -> None:
        ...
        """
        intercept_index = int(self.include_intercept_)
        self.fidelity_module_.weights = weights
        self.regularization_module_.weights = np.r_[np.zeros(intercept_index), weights[intercept_index:]]
