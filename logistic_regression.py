...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        initial_weights = np.random.randn(X.shape[1]) / np.sqrt(X.shape[1])

        if self.penalty_ == "none":
            objective_function = LogisticModule(weights=initial_weights)
        else:
            regularization_module = {"l1": L1, "l2": L2}[self.penalty_]()
            objective_function = RegularizedModule(
        ...
        self.coefs_ = self.solver_.fit(objective_function, X, y)
    ...
    def _predict(self, X: np.ndarray) -> np.ndarray:
        ...
        return (probabilities >= self.alpha_)
