...
def compare_fixed_learning_rates(init: np.ndarray = np.array([np.sqrt(2), np.e / 3]),
    ...
    for module_name, module_cls in modules.items():
        ...
        for eta in etas:
            # Set up the callback and lists for recording
            ...
            # Plot and save the descent path
            descent_path = np.array([init] + weights)
            fig = plot_descent_path(module_cls, descent_path, title=f"{module_name} - Learning Rate: {eta}")
            fig.write_image(f"Plots/gd_{module_name}_eta_{eta}.png")

        # Plot and save the norm of weights for different learning rates
        ...
        norm_fig.write_image(f"Plots/gd_{module_name}_weight_norm_convergence.png")
...
def plot_roc_curve(y_true, y_prob, title):
    ...
    fig.write_image(f"Plots/{title.replace(' ', '_').lower()}.png")
...
def plot_cross_validation_results(lambda_values, scores, penalty):
    ...
    fig.write_image(f"Plots/{penalty}_logistic_cross_validation_errors.png")
...
#
#     # Fitting l1- and l2-regularized logistic regression models, using cross-validation to specify values of
